...
class ThreeDMatchDivPairDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        dataset_root,
        subset,
    ...
    def _load_point_cloud(self, file_name):
        points = np.load(osp.join(self.data_root, file_name))
    ...
    def __getitem__(self, index):
        ...
        # get point cloud
        ref = self._load_point_cloud(metadata['pcd0'])
        src = self._load_point_cloud(metadata['pcd1'])
        ref_points = ref[:, :3]
        src_points = src[:, :3]
        if ref.shape[1] == 6:
