...
def pcd_point2anchor_partition(points, anchor_num=5, anchor_max_points=None):
...
def registration_collate_fn_split_stack_mode(
    data_dicts, num_stages, voxel_size, search_radius, neighbor_limits, anchor_num=5, precompute_data=True
):
    r"""Collate function for registration in stack mode.
    ...
    # handle special keys: [ref_feats, src_feats] -> feats, [ref_points, src_points] -> points, lengths
    # points_list = collated_dict.pop('ref_points') + collated_dict.pop('src_points')
    points_list = pcd_point2anchor_partition(collated_dict.pop('ref_points')[0], anchor_num=anchor_num) 
    points_list += pcd_point2anchor_partition(collated_dict.pop('src_points')[0], anchor_num=anchor_num)
    lengths = torch.LongTensor([points.shape[0] for points in points_list])
    points = torch.cat(points_list, dim=0)
    feats = torch.cat(collated_dict.pop('ref_feats') + collated_dict.pop('src_feats'), dim=0)
    ...
    if precompute_data:
        ...
        collated_dict['lengths'] = lengths
    collated_dict['batch_size'] = batch_size
    collated_dict['anchor_num'] = anchor_num
