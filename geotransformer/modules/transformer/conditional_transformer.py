...
class RPEConditionalTransformerSOAR(nn.Module):
    def __init__(
        self,
        blocks,
        d_model,
    ...
    def forward(self, feats0, feats1, embeddings0, embeddings1, masks0=None, masks1=None):
        ...
        for i, block in enumerate(self.blocks):
            if block == 'self':
                # feats0, scores0 = self.layers[i](feats0, feats0, embeddings0, memory_masks=masks0)
                # feats1, scores1 = self.layers[i](feats1, feats1, embeddings1, memory_masks=masks1)
                feats_score0 = [self.layers[i](f0, f0, e0, memory_masks=masks0) for f0, e0 in zip(feats0, embeddings0)]
                feats_score1 = [self.layers[i](f1, f1, e1, memory_masks=masks1) for f1, e1 in zip(feats1, embeddings1)]
                feats0, scores0 = zip(*feats_score0)
                feats1, scores1 = zip(*feats_score1)
            else:
                if self.parallel: # False
                    ...
                    # feats0, scores0 = self.layers[i](feats0, feats1, memory_masks=masks1)
                    # feats1, scores1 = self.layers[i](feats1, feats0, memory_masks=masks0)
                    feats1_full = torch.cat(feats1, dim=1)
                    feats_score0 = [self.layers[i](f0, feats1_full, memory_masks=masks1) for f0 in feats0]
                    feats0_full = torch.cat(feats0, dim=1)
                    feats_score1 = [self.layers[i](f1, feats0_full, memory_masks=masks0) for f1 in feats1]
                    feats0, scores0 = zip(*feats_score0)
                    feats1, scores1 = zip(*feats_score1)

            if self.return_attention_scores:
                attention_scores.append([scores0, scores1])
        if torch.isnan(torch.cat(feats0, dim=1)).sum() > 0:
            print('after attentions!')
            pdb.set_trace()
        if self.return_attention_scores:
            return feats0, feats1, attention_scores
