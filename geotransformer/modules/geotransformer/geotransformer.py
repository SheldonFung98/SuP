...
from geotransformer.modules.transformer import (
    SinusoidalPositionalEmbedding, 
    RPEConditionalTransformer,
    RPEConditionalTransformerSOAR,
)
...
class GeometricTransformerSOAR(nn.Module):
    def __init__(
        self,
        input_dim,
        output_dim,
        ...
        self.in_proj = nn.Linear(input_dim, hidden_dim)
        self.transformer = RPEConditionalTransformerSOAR(
            blocks, hidden_dim, num_heads, dropout=dropout, activation_fn=activation_fn
        )
        self.out_proj = nn.Linear(hidden_dim, output_dim)
    ...
    def forward(
        ...
        Returns:
            ...
            src_feats: torch.Tensor (B, M, C)
        """
        ref_embeddings = [self.embedding(p[None, ...], hsv=c[None, ...]) for p, c in zip(ref_points, ref_color)]
        src_embeddings = [self.embedding(p[None, ...], hsv=c[None, ...]) for p, c in zip(src_points, src_color)]

        ref_feats = [self.in_proj(f[None, ...]) for f in ref_feats]
        src_feats = [self.in_proj(f[None, ...]) for f in src_feats]
        ...
        ref_feats = [self.out_proj(f) for f in ref_feats]
        src_feats = [self.out_proj(f) for f in src_feats]
