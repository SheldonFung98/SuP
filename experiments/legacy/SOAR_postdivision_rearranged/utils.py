...
class PartialRegistrationGenerator(torch.nn.Module):
	...
	def sample(self, points, anchor_num=8, k=180):
		...
		# 	180
		# )
		# if (neighbor_indices==points.shape[0]).any():
		# 	print("Warning: some points are not in the neighbor_indices")
		return neighbor_indices, neighbor_indices!=points.shape[0]
	...
	def forward(self, ref_points, src_points, ref_feats, src_feats, aux=None):
		...
		with torch.no_grad():
			failed_ind = [1, 8, 36, 39, 44, 49, 53, 56, 63, 80, 89, 97]
						#[1, 8, 36, 39, 43, 44, 45, 49, 53, 56, 61, 63, 80, 89,
			# AUX data
			gt_tf, index = aux
		...
		# ref_sp_ind = ref_sp_ind.to(ref_points.device)
		# ref_sp_mask = ref_sp_mask.to(ref_points.device)
		...
		# src_sp_ind = src_sp_ind.to(src_points.device)
		# src_sp_mask = src_sp_mask.to(src_points.device)
...
class FeatureAlignmentConsistencyWeighting(torch.nn.Module):
	...
	def forward(self, ref_points, src_points, ref_feats, src_feats, tf_est):
		tfs_num = tf_est.shape[0]
		d = torch.cdist(
			ref_points[None, ...].repeat(tfs_num, 1, 1), 
			apply_transform(
				src_points[None, ...].repeat(tfs_num, 1, 1),
				tf_est
			)
		)
		batch_ind, ref_c_ind, src_c_ind = (d < self.radius).nonzero(as_tuple=True)
		ref_feats_c = ref_feats.repeat(tfs_num, 1, 1)[batch_ind, ref_c_ind]
		src_feats_c = src_feats.repeat(tfs_num, 1, 1)[batch_ind, src_c_ind]
		f_ref = self.model_f(ref_feats_c)
		f_src = self.model_f(src_feats_c)
		f_d = self.model_d(d[batch_ind, ref_c_ind, src_c_ind].unsqueeze(-1))
		f_m = self.model_m(f_ref-f_src) * f_d
		_, batch_ind_num = batch_ind.unique(return_counts=True)
		w_m = self.model_w(torch.stack([i.max(dim=0).values for i in torch.split(f_m, batch_ind_num.tolist())])).flatten()
		w_m = w_m.softmax(dim=0)
		return w_m