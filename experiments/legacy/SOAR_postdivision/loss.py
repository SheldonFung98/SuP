...
class Evaluator(nn.Module):
    ...
    @torch.no_grad()
    def evaluate_sel_registration(self, output_dict, data_dict):
        transform = data_dict['transform']
        ...
        src_points = output_dict['src_points']
        def compute_recall(est_transform):
            realignment_transform = torch.matmul(torch.inverse(transform), est_transform)
            realigned_src_points_f = apply_transform(src_points, realignment_transform)
            rmse = torch.linalg.norm(realigned_src_points_f - src_points, dim=1).mean()
            recall = torch.lt(rmse, self.acceptance_rmse).float()
            return recall
        return torch.stack([compute_recall(est_transform) for est_transform in est_transforms]).any(), \
            torch.stack([compute_recall(est_transform) for est_transform in sel_est_transforms]).any()
    ...
    def forward(self, output_dict, data_dict):
        ...
        all_recalls, sel_recall = self.evaluate_sel_registration(output_dict, data_dict)
