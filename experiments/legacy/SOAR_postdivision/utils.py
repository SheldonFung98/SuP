...
class FeatureAlignmentConsistencyWeighting(torch.nn.Module):
    ...
    def forward(self, ref_points, src_points, ref_feats, src_feats, tf_est):
        ...
        src_points_alined = apply_transform(
            src_points[None, ...].repeat(tfs_num, 1, 1),
            tf_est
        )

        batch_ind, ref_c_ind, src_c_ind = (torch.cdist(ref_points[None, ...].repeat(tfs_num, 1, 1), src_points_alined) < self.radius).nonzero(as_tuple=True)

        ref_feats_c = ref_feats[ref_c_ind]
        src_feats_c = src_feats[src_c_ind]

        _, batch_ind_num = batch_ind.unique(return_counts=True)

        ref_feats_c = torch.split(ref_feats_c, batch_ind_num.tolist())
        src_feats_c = torch.split(src_feats_c, batch_ind_num.tolist())