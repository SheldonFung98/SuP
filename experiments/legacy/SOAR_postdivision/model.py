...
class GeoTransformer(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        division = 8
        # ref_centroids = utils.weighted_farthest_point_sample(ref_points_c[None, ...], division, ref_overlap_est[None, ...])
        # src_centroids = utils.weighted_farthest_point_sample(src_points_c[None, ...], division, src_overlap_est[None, ...])

        # _, _, ref_anchor_knn_indices, ref_anchor_knn_masks = utils.point2anchor_partition(ref_points_c)
        _, _, ref_anchor_knn_indices, ref_anchor_knn_masks = utils.pcd_division_knn_radius(ref_points_c.cpu(), fragment_num=division)
        ...
        ref_padded_c_feats = torch.cat([ref_feats_c_norm, torch.zeros_like(ref_feats_c_norm[:1])], dim=0)
        ...
        _, _, src_anchor_knn_indices, src_anchor_knn_masks = utils.pcd_division_knn_radius(src_points_c.cpu(), fragment_num=division)
        ...
        src_padded_c_feats = torch.cat([src_feats_c_norm, torch.zeros_like(src_feats_c_norm[:1])], dim=0)
        ...
        if False: # view pcd division
            pcd = ref_points_c
            blocks = ref_pc_blocks
            psc = PSC().vedo(subplot=9)
            psc.add_pcd(ref_points_c)
            for ind, b in enumerate(ref_pc_blocks):
                psc.draw_at(ind+1).add_pcd(b)
            psc.show()
        ...
        block_overlap_prior_est = self.blockwise_overlap_est_list(ref_fc_blocks, src_fc_blocks)
        ...
        division_num = 24
        _, sel_ind = block_overlap_prior_est.view(-1).topk(k=division_num, largest=True)
        ...
        # Block-wise Overlap Estimation
        block_overlap_est = block_overlap_prior_est[ref_sel_ind, src_sel_ind]
        ...
        weights = torch.zeros(division_num, 5).to(ref_feats_f.device)
        weights[:, 1] = 1
        Ts = torch.eye(4, 4).repeat(division_num, 1, 1).to(ref_feats_f.device)

        # ref_feats_m_norm = F.normalize(ref_feats_m, p=2, dim=-1)
        # src_feats_m_norm = F.normalize(src_feats_m, p=2, dim=-1)
        ...
        # merged_weights = weights[:, 1] * weights[:, 2]
        # merged_weights = weights[:, 0] * weights[:, 2]
        # weights[:5, :5] = 1
        # weights[-5:, -5:] = 1
        N_cyclegraph = 100
        ...
        if False:
            ...
            psc = PSC().vedo().add_pcd(ref_points_c).add_pcd(src_points_c, T).add_pcd(src_points_c, transform).show()
