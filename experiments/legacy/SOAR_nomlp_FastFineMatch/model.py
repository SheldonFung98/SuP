...
class GeoTransformer(nn.Module):
	...
	def forward(self, data_dict):
		...
		Ts, Ts_weights, ov_weights, w_facw, ref_node_corr_indices, src_node_corr_indices, corr_scores = \
			self.prg(ref_points_c, src_points_c, ref_feats_c_norm, src_feats_c_norm, aux=(transform, data_dict['index']))


		d_num = corr_scores.shape[0]
		...
		# with torch.no_grad():
			...
			# merged_weights = w_facw * weights[:, 0] * weights[:, 1]
