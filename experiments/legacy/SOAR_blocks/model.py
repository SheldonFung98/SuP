...
class GeoTransformer(nn.Module):
    def __init__(self, cfg):
        ...
        self.transformer = GeometricTransformerSOAR(
    ...
    def pcd_overlap_est(self, ref_feats, src_feats, topk=3, mutual_topk=False):
    ...
    def blockwise_overlap_est(self, ref_feats_b, src_feats_b, topk=3, mutual_topk=False):
        ...
        block_overlap_est_norm = (block_overlap_est - block_overlap_est.min()) / block_overlap_est.max()
        return block_overlap_est_norm
    ...
    def forward(self, data_dict):
        ...
        block_overlap_prior_est = self.blockwise_overlap_est(ref_fc_blocks_norm, src_fc_blocks_norm)
        sel_ind = (block_overlap_prior_est >= block_overlap_prior_est.view(-1).topk(k=8).values[-1]).nonzero()
        ref_sel_ind, src_sel_ind = sel_ind[:, 0].unique(), sel_ind[:, 1].unique()
        ...
        ref_feats_c, src_feats_c = self.transformer(
            [ref_pc_blocks[i] for i in ref_sel_ind],
            [src_pc_blocks[i] for i in src_sel_ind],
            [ref_fc_blocks[i] for i in ref_sel_ind],
            [src_fc_blocks[i] for i in src_sel_ind],
            ref_color=[ref_hsv_blocks[i] for i in ref_sel_ind],
            src_color=[src_hsv_blocks[i] for i in src_sel_ind],
        )
        ...
        ref_feats_c_norm = [F.normalize(i.squeeze(0), p=2, dim=1) for i in ref_feats_c]
        src_feats_c_norm = [F.normalize(i.squeeze(0), p=2, dim=1) for i in src_feats_c]
        ...
        if False:
            ...
            for i in range(b_len):
                ...
                color[:, 0] = block_overlap_est[ind, i]
                psc.add_color(color)
            psc.show()
        ...
        weights = torch.zeros(len(ref_feats_c_norm), len(src_feats_c_norm)).to(ref_feats_f.device)
        Ts = torch.eye(4, 4).repeat(len(ref_feats_c_norm), len(src_feats_c_norm), 1, 1).to(ref_feats_f.device)
        ...
        for i in range(len(ref_feats_c_norm)):
            for j in range(len(src_feats_c_norm)):
                ...
                weights[i, j] = block_overlap_est[i, j]
                if corr_scores.shape[0] > 0:
                    ...
                    weights[i, j] *= torch.exp(-2*corr_weight)
                Ts[i, j] = estimated_transform
        # weights[:5, :5] = 1
        # weights[-5:, -5:] = 1
        N_cyclegraph = 100
        # Tglobals, weights_out = pair2globalT_cycle(weights.cpu(), Ts.cpu(), N_cyclegraph)

        sel_weights, sel_ind = weights.view(-1).topk(2)
        sel_Ts = Ts.view(-1, 4, 4)[sel_ind]
        output_dict['sel_estimated_transform'] = Ts
        ...
        for i in range(N_cyclegraph):
            ...
            bias = [angle_bias[i] * (apply_transform(output_dict['src_corr_points'][i], estimated_transform) - output_dict['ref_corr_points'][i]).square().sum(dim=1).sqrt().mean() for i in range(angle_bias.shape[0])]
